...
PROVIDERS: dict[str, dict] = {
    ...
    "zoom": {
        ...
        "status": "available", "icon": "zoom", "accent": "#2D8CFF",
        "description": "Distill Zoom meeting recordings into decisions & action items.",
        "captures": "Cloud recording transcripts",
        "install_path": "/zoom/install",
        # No folder/channel selection — account-wide once connected. An admin
        # authorizes and every meeting's recording.completed event is captured
        # from then on; no separate "pick what to watch" step exists for it.
        "needs_own_app": True,
        "redirect_path": "/zoom/oauth/callback",
        "webhook_path": "/zoom/events",
        "needs_webhook_secret": True,
        "setup_fields": [
            {"key": "client_id", "label": "Client ID"},
            {"key": "client_secret", "label": "Client Secret", "secret": True},
            {"key": "webhook_secret", "label": "Secret Token", "secret": True},
        ],
