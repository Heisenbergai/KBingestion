...
import re
import io
import httpx                         # downloads file via signed URL
...
# ── Clients ────────────────────────────────────────────────────────────────────
# Supabase here is only used to STORE vectors (our personal Supabase).
# File download now uses a signed URL — no Supabase storage access needed.
...
class IngestRequest(BaseModel):
    document_id: str   # id from knowledge_items table
    signed_url:  str   # temporary signed download URL generated by Lovable
    mime_type:   str   # e.g. "application/pdf"
    file_name:   str   # e.g. "Employee Handbook.pdf"
    asset_id:    str   # same as document_id


# ── Step 1: Download ───────────────────────────────────────────────────────────
def download_file(signed_url: str) -> bytes:
    """
    Downloads the raw file bytes from the signed URL.
    Lovable generates this URL from its own Supabase Storage.
    Valid for 1 hour. No credentials needed on our side.
    """
...
async def ingest_document(request: IngestRequest):
    ...
    """
    try:
        # 1. Download file via signed URL (no Supabase credentials needed)
        ...
        # 6. Build rows to insert into our personal Supabase
        rows = [
            {
